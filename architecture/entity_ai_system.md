...
## Bug Fix: Monster Visibility Issue

### Problem
Monsters were attacking the player without being visible to them. The player was being killed by monsters they couldn't see.

### Root Cause
The AI component in `src/domain/components/ai.py` was not checking the perception component before attacking. The `_handle_chasing_state` and `_update_aggressive_ai` methods would attack/move toward targets regardless of whether the entity could actually see them.

### Solution
Modified the AI component to check the perception component before attacking:

1. **`_can_see_target` method**: Now checks the `PerceptionComponent.current_status.can_see_player` first, falling back to distance-based check if no perception component exists.

2. **`_handle_chasing_state` method**: Now checks visibility before attacking. If target is not visible, switches to "searching" state.

3. **`_update_aggressive_ai` method**: Now checks visibility before chasing. If target is not visible, clears the target and goes to idle state.

### Files Modified
- `src/domain/components/ai.py`: Added visibility checks to AI behavior methods
- `tests/test_ai_visibility.py`: Added new test file with 8 tests for visibility-based AI behavior

### Test Results
All 822 tests pass, including 8 new tests specifically for AI visibility behavior.
